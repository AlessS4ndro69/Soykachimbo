...
const AreaList = () => {
    ...
    const {buyCourse, _week, coursesHolding} = useContext(UserContext);
        ...
        <FlatList
            ...
            data = {data.map(function (element){
                if(coincidencias.find((e) => e.codeCourse == element.courseCode  )){
                    return {...element, pay: true, price:  49 + Math.floor(Math.random()*(57-49)) }
                }
                else{
                    return {...element, pay: false, price:  49 + Math.floor(Math.random()*(57-49))}
                }
            ...
            renderItem={({ item }) => (
                <View style= {tw`flex-row p-2 pb-2 bg-gray-200 m-2 w-full justify-between`}> 
                    <TouchableOpacity
                    onPress={() => {
